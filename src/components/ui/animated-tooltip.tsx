    ...
    <div className={cn("flex items-center gap-2", className)}>
      {items.map((item) => {
        ...
        const wrapperProps = item.href
          ? { href: item.href, target: "_blank", rel: "noopener noreferrer", title: `${item.name} — ${item.designation}` }
        ...
        return (
          ...
          >
            ...
            {item.image ? (
              ...
              <div
                onMouseMove={handleMouseMove}
                className="flex items-center justify-center !m-0 !p-0 rounded-full h-14 w-14 border-2 bg-neutral-100 group-hover:scale-105 group-hover:z-30 border-background relative transition duration-500"
              >
                {item.icon}
              </div>
            )}
